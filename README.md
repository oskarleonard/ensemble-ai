...
| --- | --- | --- |
| `ensemble-ai review [<pr-url>]` | Self-contained cross-vendor code review — Codex + Grok + a cold Opus as blind peers, then a Claude **gate** grounds each finding (`agree`/`partial`/`false`/`unverified`) + a synthesis. | source: `--pr <N\|url>` · `--staged` · `--working-tree` · `--diff-file <p>` · stdin (default: current branch) · `--reviewers <ids>` · `--no-claude` · gate: `--strict-high` · `--gate-dismissals` · `--gate-model`/`--gate-effort` · `--claude-model`/`--claude-effort` · `--no-fail-on-high` · **`--stage`** (stage a PENDING review; needs a PR **URL**) · `--post-comment` (deprecated) · `--out <dir>` |
