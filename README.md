...
- **Severity capped at MED, and only a *citation* lifts it.** The cap is exceeded only when the verdict carries a citation of a **conventions doc** that the host locates verbatim at `headSha` (and that is really a conventions doc, not a README). A model asserting importance never uncaps — the check is code, not a request.
