...
type OrphanGitProbe = () => boolean;
declare function hasOrphanedGitChild(psOutput: string, parentDead: (pid: number) => boolean): boolean;
declare function orphanedGitChildrenExist(): boolean;
declare const DEFAULT_LOCK_STALE_MS: number;
declare function touchRepoLock(gitCommonDir: string): void;
interface LockOpts {
    orphanProbe?: OrphanGitProbe;
    retries?: number;
    sleepMs?: number;
    staleMs?: number;
}
declare function acquireRepoLock(gitCommonDir: string, opts?: LockOpts): () => void;
declare function acquireRepoLockAsync(gitCommonDir: string, opts?: LockOpts): Promise<() => void>;
